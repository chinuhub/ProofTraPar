...
namespace boost
{
  ...
  template < typename G > void warshall_transitive_closure(G & g, std::vector<std::set<typename graph_traits<G>::vertex_descriptor>>& adjlist)
    {
      ...
      for (boost::tie(ki, ke) = vertices(g); ki != ke; ++ki){
              for (boost::tie(ii, ie) = vertices(g); ii != ie; ++ii){
            	  if (edge(*ki, *ii, g).second)
            	  {
            		  //std::string vernb=boost::get(boost::vertex_name,g,en.m_target);
            		  adjlist[boost::get(boost::vertex_index,g,*ki)].insert(*ii);
            	  }
              }
      }
          ...
          if (edge(*ii, *ki, g).second)
            for (boost::tie(ji, je) = vertices(g); ji != je; ++ji)
              if (!edge(*ii, *ji, g).second && edge(*ki, *ji, g).second) {
            	  adjlist[boost::get(boost::vertex_index,g,*ii)].push_back(*ji);
              }
*/
      for (boost::tie(ii, ie) = vertices(g); ii != ie; ++ii)
      {
    	  std::set<typename graph_traits<G>::vertex_descriptor> nxti= adjlist[boost::get(boost::vertex_index,g,*ii)];
    	  for (boost::tie(ki, ke) = vertices(g); ki != ke; ++ki)
              {
    		  std::set<typename graph_traits<G>::vertex_descriptor> nxtk= adjlist[boost::get(boost::vertex_index,g,*ki)];
            	  if(nxtk.find(*ii)!=nxtk.end())
            	  {//means an edge from *ii to *ki

            		  for (boost::tie(ji, je) = vertices(g); ji != je; ++ji)
            		  {
            			  if(nxti.find(*ji)!=nxti.end()){
            				  adjlist[boost::get(boost::vertex_index,g,*ki)].insert(*ji);
            			  }
            		  }
