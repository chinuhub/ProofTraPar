...
Generate symbol-table and all the needed data structures.
(this module is used at the beginning of all other modules for building basic data structs about the input code)
...
Prerequisites:
	...
	- no A->b, should be (*A).b instead (see workaround no. 4)
...
class Parser(pycparser.c_generator.CGenerator):
	...
	def reset(self):
		...
		self.currentFunct = ''         # name of the function which definition is being parsed ('' = none)
	...
	def visit_UnaryOp(self, n):
		...
		if n.op == 'p++': return '%s++' % operand
			...
			if self.nodecoords[n]:
				if operand in self.varNames[self.currentFunct]:
					self.varDeReferenced[self.currentFunct,operand].append(int(self.nodecoords[n][1:]))
				elif operand in self.varNames['']:
					self.varDeReferenced['',operand].append(int(self.nodecoords[n][1:]))

			return '%s%s' % (n.op, operand)
		elif n.op == '&':
			#print "REFERENCING %s (line:%s)" % (operand, self.nodecoords[n]);
			if self.nodecoords[n]:
				if operand in self.varNames[self.currentFunct]:
					self.varReferenced[self.currentFunct,operand].append(int(self.nodecoords[n][1:]))
				elif operand in self.varNames['']:
					self.varReferenced['',operand].append(int(self.nodecoords[n][1:]))
