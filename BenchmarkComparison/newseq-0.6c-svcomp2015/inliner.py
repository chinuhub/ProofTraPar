VERSION = 'inliner-0.0-2014.10.31'
...
Transformations:
	- inlining of all the function calls, for functions which body is defined (except main() and __VERIFIER_atomic_ functions)
...
class Inliner(module.Module):
	...
	def visit_FuncDef(self, n):
		...
		if (self.currentFunction[-1] in self.Parser.threadName or self.currentFunction[-1] == 'main'):
			returnStmt = self.INDENT_SPACING + '__exit_%s: ; %s(0);\n' % (self.currentFunction[-1], common.changeID['pthread_exit'])
	...
	def visit_FuncCall(self, n):
		...
		#
		if fref == common.changeID['pthread_exit']:  # pthread_exit() param is ignored for the moment
		...
		if self.____needsInlining(fref):			
			...
			else: s =  '__retval_%s' % (self.indexStack[-1])
	...
	def visit_Return(self, n):
		if len(self.indexStack) > 0:
			if self.Parser.funcIsVoid[self.currentFunction[-1]]:
				...
				return '__retval_%s = %s; goto __exit_%s;' % (self.indexStack[-1], self.visit(n.expr), self.indexStack[-1]) # non-void
	...
	def visit_Decl(self, n, no_type=False):
		...
		if (isinstance(n, c_ast.Decl) and
			...
			if (self.__isScalar(self.currentFunction[-1], n.name) or self.__isStruct(self.currentFunction[-1], n.name)) and not self.Parser.varInitExpr[self.currentFunction[-1], n.name]:
				...
				#s += n.name + ' = (%s %s)malloc(sizeof(%s)*%s); assume(%s)' % (vartype, stars, vartype, self._totalSize(self.currentFunction[-1], n.name), n.name)
	...
	def _inlineFunction(self, n, fcall_ast_node, simple):
		...
		inlined = inlined.replace(self.INDENT_SPACING+'{', self.INDENT_SPACING+fOutput + self._make_indent() +'{\n' + self._make_indent(1) +fInput, 1)
		...
		# void and non-void returns, respectively
		'''
		inlined = inlined.replace('return;', 'goto __exit_%s;' % ( self.indexStack[-1]  )) # void
		inlined = re.sub(r'return (.+);', r'__retval_%s = \1; goto __exit_%s;' % (self.indexStack[-1], self.indexStack[-1]), inlined) # non-void
		'''
	...
	def ____needsInlining(self, f):
		return (f in self.Parser.funcBlock and    # defined functions need to be inlined when called (if at all)
		        ...
		        not f.startswith(common.funcPrefixChange['__VERIFIER_atomic']) and 
